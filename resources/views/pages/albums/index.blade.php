        ...
        timeout = setTimeout(() => {
            ...
            fetch(`/albums/search?q=${encodeURIComponent(q)}&sort=${sort}`)
                ...
                .then(data => {
                    ...
                    if (data.length === 0) {
                        const importUrl = `{{ route('albums.import.form') }}?q=${encodeURIComponent(q)}`;
                        results.innerHTML = `
                    ...
                    data.forEach(album => {
                        const coverImg = album.cover_url 
                            ? `<img src="${album.cover_url}" alt="${album.title}" class="w-14 h-14 object-cover rounded border border-border bg-background" onerror="this.onerror=null; this.parentElement.innerHTML='<div class=\\'w-14 h-14 bg-gradient-to-br from-gray-100 to-gray-200 rounded flex items-center justify-center\\'><svg class=\\'w-5 h-5 text-gray-400\\' fill=\\'none\\' stroke=\\'currentColor\\' viewBox=\\'0 0 24 24\\'><path stroke-linecap=\\'round\\' stroke-linejoin=\\'round\\' stroke-width=\\'1.5\\' d=\\'M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3\\'></path></svg></div>'">`
                                   ...
                                   class="bg-card border border-border rounded-lg hover:shadow-sm transition overflow-hidden block group p-2">
                                    <div class="flex gap-3 items-center">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <h3 class="font-semibold text-base text-foreground truncate group-hover:text-primary transition">${album.title}</h3>
