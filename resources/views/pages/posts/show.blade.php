    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-12">
        ...
        <div class="bg-card border border-border rounded-lg p-6 shadow-sm">
            <div class="flex justify-between items-start mb-6">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-foreground mb-2">{{ $post->title }}</h1>
                    @if ($post->group)
                ...
                @if(!$post->isDeleted() && (auth()->id() === $post->ownerUser->id || ($post->group && auth()->id() === $post->group->owner) || (auth()->check() && auth()->user()->is_admin)))
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" class="text-muted-foreground hover:text-foreground transition-colors p-1 rounded hover:bg-muted">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z"/>
                        </svg>
                    </button>
                    <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-48 bg-card rounded-md shadow-lg border border-border z-10" x-cloak>
                        <div class="py-1">
            ...
            <div class="prose prose-lg max-w-none mb-8">
                <p class="text-foreground whitespace-pre-line leading-relaxed text-lg">{{ $post->description }}</p>
            </div>
            ...
            <div class="flex justify-between items-center text-sm text-muted-foreground pt-6 border-t border-border">
                <div class="flex gap-6 flex-1">
                    ...
                    @else
                        <button class="{{ auth()->user()?->is_blocked ? 'opacity-50 cursor-not-allowed' : 'reaction-btn hover:text-blue-600' }} flex items-center gap-1 transition-colors"
                                data-post-id="{{ $post->id }}" data-type="like" @if(auth()->user()?->is_blocked) disabled @endif>
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>
                            <span class="likes-count">{{ $post->likes }}</span>
                        </button>

                        <button class="{{ auth()->user()?->is_blocked ? 'opacity-50 cursor-not-allowed' : 'reaction-btn hover:text-yellow-600' }} flex items-center gap-1 transition-colors"
                                data-post-id="{{ $post->id }}" data-type="confetti" @if(auth()->user()?->is_blocked) disabled @endif>
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 2v4M12 22v-4M2 12h4M22 12h-4"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M5 5l3 3M19 5l-3 3M5 19l3-3M19 19l-3-3"/></svg>
                ...
                <button class="flex items-center gap-1.5 text-muted-foreground hover:text-red-00 transition-colors text-xs" onclick="openReportModal('report-modal-{{ $post->id }}')">
                ...
                @auth
                    @if(!$post->isDeleted())
                        @if(auth()->user()->is_blocked)
                            <div class="mb-6 p-4 bg-red-50 border border-red-100 rounded-lg text-center">
                                <p class="text-red-400 text-sm font-medium italic text-center">Your account is blocked. You cannot comment.</p>
                            </div>
                        @else
